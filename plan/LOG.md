...
- [x] FE-5 Shareable metadata/OG (M): server `generateMetadata` on /v, /posts, /u (each split into server `page.tsx` + `*Client.tsx`, client behavior incl. `?tab=` unchanged); new `lib/api/serverBase.ts` (SSR base resolver: prod=absolute `NEXT_PUBLIC_API_BASE_URL`, dev=`BACKEND_ORIGIN`/:8010; `serverFetch` never throws→falls back to `{title:"CeCeCar"}`; `absoluteMediaUrl` relative `/media`→`https://cececar.com`, drops `data:` avatars); `metadataBase` in layout; twitter `summary_large_image` (F11). Committed `1e8b4be`. Verified: tsc clean, live curl shows og: tags for vehicle/post/user w/ absolutized media, bogus/private ids → 200 + default metadata.
- [x] FE-6 A11y + polish + rebrand (S–M): modal focus trap via `lib/useDialogFocus.ts` (Esc + focus restore) wired into UserListModal + Lightbox (F13); polish bundle incl. `app/not-found.tsx`, carousel/lightbox controls, alt text, visitor specs (F19); rebrand "Car Social"→**CeCeCar** (correct three-capital-C casing) in layout title, Nav alt, auth header (F17); dropped dead `images.remotePatterns` from next.config (no next/image). Committed `3cd75e4`. Verified: tsc clean, running-app title=CeCeCar, not-found renders, no "Car Social"/"CeceCar" left in source. (.tab adopt/remove F16 deferred into FE-7 cleanup.)
- [x] FE-7 Cleanup/refactor (S): `lib/useMe.ts` centralizes the `["me"]` query (replaced identical inline `useQuery` at 8 call sites; ProfileEditor invalidations untouched) (F15); PostCard like state now derives from the cached post prop via an optimistic mutation that patches every cache holding it (`["post",id]`, `["feed"]` InfiniteData, `["vehiclePosts"]`, `["userPosts"]`) w/ idempotent mapper + rollback — survives virtualized-feed remounts, guest-guard + no-double-POST preserved (F8); `.tab` already adopted in VehicleClient, no change (F16). Committed `dbc7d4c`. Verified: tsc clean, no leftover inline `["me"]` query, feed 200. **FE-1→FE-7 hardening backlog now fully cleared.**
...
### History
- **Build sheet / mods list** (Phase 1 moat feature): a categorized, current-state list of a vehicle's modifications, distinct from the chronological History tab. New `vehicle_mods` table + Alembic `0005_vehicle_mods` (category/name/brand/`cost_cents`+currency/link/`installed_date`/notes/`sort_order`, soft-delete; no category CHECK — frontend "Other…" escape hatch; no per-mod visibility, inherits vehicle). Owner-gated CRUD: `POST|GET /vehicles/{id}/mods`, `PATCH|DELETE /mods/{id}`; `delete_vehicle` now removes mods first (same class of fix as `60de96d`). Frontend: a **Build** tab on `/v` (between History and Specs, URL-shareable `?tab=build`), category-grouped rows (consecutive-run grouping over the backend's category-sorted list) with empty fields hidden, owner Add/Edit/Delete (delete-confirm), read-only for visitors, empty state; new `VehicleModForm` (dollars↔cents, category dropdown + Other hatch, edit prefill). Backend `2048005` (pytest 11 passed, migration up/down round-trips), frontend `eb19ace` (tsc clean, backend CRUD curl-verified end-to-end; interactive render reasoned vs the History tab — no headless browser this session). Not yet in the ZIP export (candidate next).
