...
## Code / data
- **[FIXED 2026-06-12] `DELETE /vehicles/{id}` 500'd when the vehicle had any `vehicle_events` rows.** (Events are soft-deleted so rows remained; no FK cascade → `db.delete(vehicle)` NULL'd `vehicle_events.vehicle_id` → NotNullViolation.) Fixed in `services.delete_vehicle` (commit on `improvements`): explicitly delete the vehicle's `vehicle_event_media`, `vehicle_event_documents`, `vehicle_events`, and `post_vehicle_tags` rows before `db.delete(vehicle)`. Tagged posts survive (only the tag is removed). Verified: vehicle-with-event → 204, no orphan rows; tagged post still 200. NOTE: still a hard delete (not soft) — fine for now. `follows.followed_vehicle_id` rows aren't cleaned up, but follow-a-vehicle has no API/rows yet — handle if/when it ships.
- (leftover dev data) FE-3's verification left `review-*` rows in the dev DB (5 vehicles by nickname, 4 events, 5 posts, dated 2026-06-11) — harmless, but clean up when convenient. FE-4's own review data was removed.
