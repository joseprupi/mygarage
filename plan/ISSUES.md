...
## Code / data
- **`DELETE /vehicles/{id}` 500s when the vehicle has any `vehicle_events` rows — including soft-deleted ones.** Events are soft-deleted (`deleted_at` set, row kept), the `Vehicle.events` relationship has no delete cascade, so `db.delete(vehicle)` makes SQLAlchemy NULL `vehicle_events.vehicle_id` → `NotNullViolation`. Found 2026-06-12 while verifying FE-4; the new delete-vehicle UI surfaces the error cleanly but the delete only works for vehicles that never had events. Backend fix needed: `cascade="all, delete-orphan"` on the relationship (and ditto media/documents/tags) or explicitly delete child rows in `services.delete_vehicle`.
