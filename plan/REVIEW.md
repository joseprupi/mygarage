...
- [ ] **FE-4 Missing CRUD** (S–M): F2 (delete event/vehicle w/ confirm), F4 (edit profile fields), F7 (require content to publish), F20 (add VIN input to vehicle form — owner-requested; vin is in schema + Specs but has no form field).
- [ ] **FE-5 Shareable metadata** (M): F11 (generateMetadata/OG for /v, /posts, /u).
- [ ] **FE-6 A11y + polish + rebrand** (S–M): F13 (dialog/focus), F19 bundle, F16 (adopt/remove .tab), F17 (Car Social → My Garage).
